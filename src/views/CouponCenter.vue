<template>
  <div class="coupon-center-view page">
    <div class="shop-view-banner">
      <div v-if="shop.photo2" class="banner-box">
        <img :src="shop.photo2" alt="门店图片" class="shop-img">
      </div>
      <div class="shop-detail hairline">
        <h3>{{ shop.name }}</h3>
        <p>营业时间：{{ shop.bizTime }}</p>
      </div>
    ...
    <coupon-list2 :coupons="coupons" v-on:getCoupon="handleGetCoupon" :loading="couponLoading"></coupon-list2>
...
import { listCouponByShop, addUserCoupon, listUserCoupon, listUserByCouponId, getShop } from '../api/api'
...
export default {
  ...
  methods: {
    listCoupon () {
      ...
      listUserCoupon().then(data => {
        ...
        return listCouponByShop(this.shopId)
      }).then(data => {
    ...
    async listCouponUser () {
      let resp = await listCouponByShop(this.shopId)
      let coupons = resp.data.coupons
      let users = []
      for (let c of coupons) {
        resp = await listUserByCouponId(c.couponId)
        let _users = resp.data.users
        users.push(..._users)
      }
    ...
    addCoupon () {
      addUserCoupon(this.currentCoupon.couponId).then(data => {
        this.$toast('领取成功！你可以在个人中心查看你的积分，下单的时候可以使用哦！')
      }).catch(e => {
        if (e.response.status === 409) {
          this.$toast('积分已经领完了')
        } else if (e.response.status === 403) {
          this.$toast('你已经领过该券，还没使用哦')
        }else {
          this.$toast(`积分不存在`)
        }
      })
    ...
    getShopInfo () {
      getShop(this.shopId).then(data => {
        ...
        shop.bizTime = shop.bizTime ? shop.bizTime : '10:00-23:00'
  ...
  mounted () {
    let { shopId, lastView } = this.$route.params
    if (lastView === 'RateForm') {
      this.$toast(`评论提交成功！请分享并领取积分`)
    }
